...
- 已使用过的方法清单（新方法用毕后追加到此）：
  ...
  16. 核心平面重入/宿主边界对抗（nested-batch×digest、mid-recompute 生命周期、自定义 scheduleRecompute sync/defer、object-atom proxy 键域、RxTime 多入口 disposer 交错、AsyncRxSlice destroy×在途 fetch）。动态复现：`isPlainObject` 以 `constructor===Object|Array` 门控导致 `Object.create(null)` 与 `constructor` 键覆写后 object-atom 属性读失明且无法建立订阅；AsyncRxSlice destroy 不抬升 `fetchReceipt`/不复位控制 atom，在途 update.reject 僵尸写入 `loadError`、fetch.finally 僵尸清 `isLoading`。未发现反例：嵌套 batch / digest 内 batch 的 sessionDepth 配平、mid-recompute create/destroy、RxTime resolve+subscribe 多入口清理（方法 13 修复后）。资产：`__tests__/_coreReview16*.spec.ts`。
