...
- 已使用过的方法清单（新方法用毕后追加到此）：
  ...
  16. 宿主语义差分回放（host-semantic differential replay）+ 核心平面重入边界：按 axii/axle RxListHost 真实消费面（splice=`argv` 原始透传 + `methodResult` 删除项、reorder=`argv[0]`+reorderInfo、EXPLICIT_KEY_CHANGE=`key`+`newValue`）维护平行 `hosts[]`，断言每步（含 batch 多 info、派生链作 hosted source、map/filter 回调重入、digest 内 batch、嵌套 batch、中途 destroy sibling）`hosts ≡ source.data`。对照"axii 现状"（EKC 读 `source.data[key]`）与"协议正确"（`info.newValue`）：前者在 batch `set`+结构操作下动态分叉，后者全程对齐——协议字段充足，分叉属下游消费脚枪；data0 侧未发现终态错误/永久毒化/泄漏反例。同轮核心平面对抗动态复现：`isPlainObject` 门控导致 `Object.create(null)`/`constructor` 覆写后 object-atom 属性读失明；AsyncRxSlice destroy 不抬升 receipt/不复位控制 atom 致在途 reject 僵尸写 `loadError`、finally 僵尸清 `isLoading`。嵌套 batch/digest 内 batch 的 sessionDepth 配平、mid-recompute create/destroy、RxTime 多入口清理未发现反例。证据：`/tmp/data0-review-repros/_reviewRound16*.spec.ts`；核心平面：`__tests__/_coreReview16*.spec.ts`。
