...
const SPARSE_FUZZ = 'sparseOpsFuzz.spec.ts' // 形态操作组合 fuzz(sweep 的生成器化升级,2026-H3 round3)
...
export const INVENTORY: Record<string, Row> = {
    ...
    'RxList.createSelection':         row('createSelection.spec.ts(示例级)', H2, 'collectionLedgerBurndown.spec.ts(undefined item)', `${SWEEP}+${SPARSE_FUZZ}`, MODEL, DESTROY, 'collectionLedgerBurndown.spec.ts(NaN item 含孪生行,RxSet 多选)+deepReview2026H3Round6.spec.ts(atom 单选 NaN 全入口:R6-1)'),
    'RxList.createSelections':        row('createSelection.spec.ts(示例级)', H2, 'collectionLedgerBurndown2.spec.ts(undefined item)', 'collectionLedgerBurndown2.spec.ts', 'collectionLedgerBurndown.spec.ts(batch splice+选中集)', 'collectionLedgerBurndown.spec.ts(含 autoReset 清理)', 'collectionLedgerBurndown2.spec.ts(NaN 孪生行)+deepReview2026H3Round6.spec.ts(atom 列继承 R6-1)'),
    'RxList.createIndexKeySelection': row('createSelection.spec.ts(示例级)', 'NA:按 index 键(与元素值无关)', 'collectionLedgerBurndown.spec.ts(undefined 行内容)', `${SWEEP}+${SPARSE_FUZZ}(校正循环洞位回归另见 deepReview2026H3Round3.spec.ts)`, 'collectionLedgerBurndown.spec.ts(batch 不等长 splice)', DESTROY, 'collectionLedgerBurndown2.spec.ts(NaN/-0 行内容)'),
