...
export const INVENTORY: Record<string, Row> = {
    ...
    'RxList.indexBy':        row('rxList.spec.ts(示例级)', 'NA:key 唯一性契约(重复 key 断言拒绝)', 'UNCOVERED', SWEEP, 'collectionLedgerBurndown.spec.ts(删+插同 key)', DESTROY),
    'RxList.toMap':          row('rxList.spec.ts(示例级)', 'NA:key 唯一性契约(重复 key 断言拒绝)', 'UNCOVERED', 'UNCOVERED', 'collectionLedgerBurndown.spec.ts(删+插同 key)', 'collectionLedgerBurndown.spec.ts(僵尸检查)'),
    'RxList.reduce':         row('reduceOperator.spec.ts(示例级差分)', 'reduceOperator.spec.ts(示例级差分)', 'UNCOVERED', 'UNCOVERED', 'collectionLedgerBurndown.spec.ts(batch 尾追加/混合)', 'reduceOperator.spec.ts(僵尸检查)'),
    ...
    'RxList.createSelections':        row('createSelection.spec.ts(示例级)', H2, 'UNCOVERED', 'UNCOVERED', 'collectionLedgerBurndown.spec.ts(batch splice+选中集)', 'collectionLedgerBurndown.spec.ts(含 autoReset 清理)'),
    'RxList.createIndexKeySelection': row('createSelection.spec.ts(示例级)', 'NA:按 index 键(与元素值无关)', 'collectionLedgerBurndown.spec.ts(undefined 行内容)', SWEEP, 'collectionLedgerBurndown.spec.ts(batch 不等长 splice)', DESTROY),
    ...
    'RxMap.values':  row(BROAD, 'collectionLedgerBurndown.spec.ts(同 value 多 key)', 'collectionLedgerBurndown.spec.ts(undefined value)', 'NA:非数组结构', MODEL, DESTROY),
    'RxMap.entries': row(BROAD, 'collectionLedgerBurndown.spec.ts(同 value 多 key)', 'collectionLedgerBurndown.spec.ts(undefined value)', 'NA:非数组结构', MODEL, DESTROY),
    ...
    'RxSet.intersection':        row(`${BROAD}(replace 含重复入参)`, 'NA:Set 成员构造性唯一', 'collectionLedgerBurndown.spec.ts(undefined 成员)', 'NA:非数组结构', MODEL, DESTROY),
    'RxSet.symmetricDifference': row(`${BROAD}(replace 含重复入参)`, 'NA:Set 成员构造性唯一', 'collectionLedgerBurndown.spec.ts(undefined 成员)', 'NA:非数组结构', MODEL, DESTROY),
    'RxSet.union':               row(`${BROAD}(replace 含重复入参)`, 'NA:Set 成员构造性唯一', 'collectionLedgerBurndown.spec.ts(undefined 成员)', 'NA:非数组结构', MODEL, DESTROY),
    ...
    'RxSet.has':                 row('rxSet.spec.ts(示例级)', 'NA:Set 成员构造性唯一', 'UNCOVERED', 'NA:非数组结构', 'UNCOVERED', 'collectionLedgerBurndown.spec.ts(随宿主销毁)'),
    ...
    'RxSet.isSupersetOf':        row('via isSubsetOf: rxSet.spec.ts(示例级)', 'NA:Set 成员构造性唯一', 'UNCOVERED', 'NA:非数组结构', 'UNCOVERED', 'collectionLedgerBurndown.spec.ts(宿主销毁后源变更安全)'),
