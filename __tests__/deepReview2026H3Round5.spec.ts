...
describe('R5-D1 载荷所有权:返回数组归调用方,协议载荷持独立副本', () => {
    ...
    test('onChange handler 拿到防御副本:改写不毒化兄弟订阅者(订阅序无关)', () => {
        // handler 先订阅(消费序在 groupBy 之前),改写载荷曾直接毒化 groupBy patch
        const list = new RxList<number>([1, 2, 3, 4])
        const disposeFn = onChange(list, (infos: any[]) => {
            const mr = infos[0]?.methodResult as number[] | undefined
            if (Array.isArray(mr)) mr.length = 0
            const argv = infos[0]?.argv as any[] | undefined
            if (Array.isArray(argv)) argv.length = 0
        })
        const groups = list.groupBy(x => x % 2)
        list.splice(0, 2)
        expect([...groups.data.get(0)!.data]).toEqual([4])
        expect([...groups.data.get(1)!.data]).toEqual([3])
        disposeFn()
        groups.destroy(); list.destroy()
    })

    test('RxSet.replace 的 onChange 副本嵌套一层:改写内层 [newItems,deletedItems] 不毒化 toList', () => {
        const set = new RxSet<number>([1, 2])
        const disposeFn = onChange(set, (infos: any[]) => {
            const mr = infos.find(i => i.method === 'replace')?.methodResult as [number[], number[]] | undefined
            if (mr) { mr[0].length = 0; mr[1].length = 0 }
        })
        const asList = set.toList()
        set.replace([2, 3])
        expect([...asList.data].sort()).toEqual([2, 3])
