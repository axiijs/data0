...
describe('R8-2 观察出口敌意 handler(onChange 先注册,深改写自己的副本)', () => {
    ...
    test('RxMap clear/replace 的 entry 对跨观察者独立(拷贝深度 mutation 盲区补杀)', () => {
        // clear 的 methodResult(entries)在 data0 内无 patch 消费者(keys 对 clear
        // 恒回退全量),协议容器的一层拷贝**只在跨双观察者形态可观察**——
        // 把 'clear' 从一层拷贝名单里变异掉(外层拷,entry 对共享)时,
        // 先注册 handler 清空 entry 对会毒化后注册 handler。replace 同理。
        const map = new RxMap<string, number>([['a', 1], ['b', 2]])
        const stopHostile = onChange(map, (infos: TriggerInfo[]) => {
            for (const info of infos) {
                if (Array.isArray(info.methodResult)) {
                    for (const el of info.methodResult) {
                        if (Array.isArray(el)) el.length = 0
        ...
        const seen: TriggerInfo[] = []
        const stopWatch = onChange(map, (infos: TriggerInfo[]) => { seen.push(...infos) })

        map.clear()
        const clearInfo = seen.find(info => info.method === 'clear')!
        expect(clearInfo.methodResult).toEqual([['a', 1], ['b', 2]])

        map.replace({x: 7})
        map.replace({y: 8})
        const replaceInfo = seen.filter(info => info.method === 'replace').at(-1)!
        expect(replaceInfo.methodResult).toEqual([['x', 7]])   // 被删 entry 完好

        stopHostile(); stopWatch(); map.destroy()
    })
})
...
describe('R8-8 batch 内非收敛效应环的可诊断性', () => {
    ...
    test('小初始队列 + 长合法级联(pending-list 处理模式):零误报(阈值 Math.max 侧 mutation 盲区补杀)', () => {
        if (!__DEV__) return
        // 初始队列 1、级联 200 步的收敛链是 handleTriggered 注释里明示支持的
        // 用户模式("不断将 pending 取出变成 processing")。阈值若被变异成
        // min(初始×16, 100k)(或倍数缩水),16 步就会对合法级联打出
        // "non-converging cycle" 误报——误报方向是用户可见噪声,必须钉死。
        const CHAIN = 200
        const atoms = Array.from({length: CHAIN + 1}, (_, i) => atom(i === 0 ? 0 : -1))
        const effects = atoms.slice(0, CHAIN).map((a, i) => {
            const eff = new Computed(function (this: Computed) {
                const v = a()
                if (v > 0) atoms[i + 1](v + 1)
            }, undefined, true)
            eff.run([], true)
            return eff
        })
        batch(() => { atoms[0](1) })   // 初始队列恰 1 个 effect,级联 200 步收敛
        expect(atoms[CHAIN].raw).toBe(CHAIN + 1)
        const cycleWarnings = errSpy.mock.calls.filter(call => String(call[0]).includes('non-converging effect cycle'))
        expect(cycleWarnings.length).toBe(0)
