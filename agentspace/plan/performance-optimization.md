...
> 状态:**Track A/B/D 已全部实施完成**,执行结果见文末 §8;Track C 维持"独立实验分支"结论,未实施。
> 基线环境:Node v22.14.0 / Linux x64,commit `ea73b4f`(v2.2.0),生产构建(`__DEV__: false`)。
> 本文 §1–§7 为实施前的方案原文,行号以基线 commit 为准(实施后部分行号已变化)。
...
## 8. 实施结果(Track A/B/D 完成后实测)

> 测量环境与基线一致(Node v22.14.0,生产构建);speed/GC 来自 `scripts/measure-speed.mjs`
> (已扩充,含每用例 minor GC 计数),内存来自 `scripts/measure-retained.mjs`,
> µs 级数据来自 `pnpm vitest bench`。

### 8.1 量化结果 vs 验收目标

| 指标 | 基线(v2.2.0) | 实施后 | 目标 | 结论 |
|---|---|---|---|---|
| atom 写,1 订阅者(2M 次) | 493ms / ≈246ns | **157ms / ≈78ns** | ≤100ns | 超额达成(3.1×) |
| 上述场景 minor GC 次数 | 1309¹ | **4** | ≤150 | 超额达成 |
| batch 写 100 atoms/轮(20k 轮) | 740ms / 340ns每写 | **252ms / 126ns每写** | ≤150ns | 达成(2.9×) |
| chain of 10 传播(vitest bench) | 13.8µs | **8.9µs** | ≤7µs | 接近(1.55×)² |
| chain of 50 传播(measure-speed,20k 写) | 700ms | **207ms** | — | 3.4× |
| create+destroy RxList(100)+map(vitest bench) | 157µs | **27µs** | ≤60µs | 超额达成(5.8×) |
| RxList(1000).map create+destroy(500 次) | 601ms | **54ms** | — | 11× |
| at() tracked ×100 | 18.2µs | **17.4µs** | ≤10µs | 未达³ |
| forEach(1000) in computed + splice 中段(2k 次) | 129ms(基线扩展 bench) | **11ms** | — | 12× |
| 稀疏订阅列表 shift+unshift ×500(10k 行,1 个 at() 订阅) | 801ms | **13ms** | — | 62×(B2) |
| RxMap create+destroy(50k 次) | 1402ms | **55ms** | — | 25× |
| computed 常驻内存 | 1083B | **947B** | ≤850B | 部分达成⁴ |
| bare computed 常驻内存 | 627B | **490B** | — | -22% |
| RxList(empty) 常驻内存 | 1793B | **274B** | ≤1100B | 超额达成(6.5×) |
| 10 万行 replaceData | RangeError 崩溃 | **正常,远低于 50ms** | 正常 | 达成(B6) |
| data0 `__tests__` | 198 全绿 | **207 全绿**(新增 9 个回归) | 全绿 | 达成 |
| axii 测试(浏览器 599 用例,经 sibling alias 用本分支 data0 源码) | 1 失败⁵ | **1 失败⁵(同一用例)** | 全绿 | 达成(无回归) |
| axle 测试(402 用例,alias 到本分支 data0) | — | **402 全绿** | 全绿 | 达成 |

¹ 基线的 1309 次是首轮 `--trace-gc` 直测;扩充后的 measure-speed(GC observer 采样)在基线上测得 12 次/2M(观测方法差异),同方法对比:12 → 4。
² chain-10 的剩余成本主要是 AtomComputed 每层 `finishFullRecompute` 的固定流程(session 建立/消化、updatedAt、dispatch),属于 Track C(版本计数/惰性求值)的范围。
³ at() tracked 的成本大头在 `track()` 的 targetMap 两级 Map 查找与 marker 位运算,本轮未动(Track C2 版本计数的目标);dispatch 守卫带来的改善约 5%。
⁴ computed 剩余 947B 中约 300B 是 `computedToInternal` WeakMap entry + data atom(AtomComputed 固有结构),再往下同样需要 Track C 级别的结构调整。
⁵ axii 的 `errorHandling.spec.tsx > adds RxList patch information to AxiiError reactive traces` 在 **data0 main 分支下同样失败**(已对照验证),是 axii 侧既有问题,与本分支无关。

### 8.2 已落地的改动清单(每项一个 commit)

- **D1**:`scripts/measure-speed.mjs` 扩充(拓扑传播/迭代粒度/派生结构用例 + 每用例 minor GC 计数);新增 `bench/structures.bench.ts`。
- **A2**:`dispatch` 单参化(rest 数组消除);track/trigger 派发点先查 `hasListener` 再构造 payload。
- **A3**:status 惰性 atom 化(内部状态机走 `_status` 数字字段);cleanPromise 惰性(`_cleanExpected` 布尔 + 读取时创建,顺带修复无人持有时 async 出错的 unhandled rejection);`Date.now()`;async 代次 id 用自增整数;调度器第三参仅在声明时拷贝。
- **A6**:`.at(-1)` → 索引访问;模块级 `notifier` 单例引用;`currentTrackFrame` 游标;triggerEffects 循环外缓存栈顶;RxList.map applyPatch 的 assert 进 `__DEV__`。
- **A1+A4+A5**:trigger 协议重构——info 由 effect 侧按需构造(`runFromTrigger`/`runFromAtomTrigger` + `needsTriggerInfo`),primitive atom 写全程标量传参;CompactDep 暴露 single/overflow,单订阅者直接派发;多 dep 用单个 Set 去重(修正旧实现"数组+临时 Set+再展开"的三重拷贝);session 改数组队列 + effect 实例字段(`_inSession`/`_sessionInfos`),空 session 快出口。
- **B1**:forEach/iterator 只 track ITERATE_KEY(所有变更路径都必然通知它,逐 index track 是纯冗余);**顺带修复了手写 iterator 从不前进导致 for...of 死循环的存量 bug**(基线可复现,由新增 bench 暴露)。
- **B2**:`pruneIndexKeyDeps` 惰性清扫空 index dep(修复"读过一次 at() 永久走慢路径"的悬崖);splice/reorder 只对受影响区间内**实际有订阅**的 index 触发 SET,O(移动范围) → O(订阅数)。
- **B6**:`spliceArray`(公开数组参数版)+ `util.spliceMany`(大批量走 copyWithin),内部所有批量写入不再 spread 传参;`argv` 形状不变(下游 axii `RxListHost` 协议不受影响)。
- **B5**:`ReactiveEffect.createDetached` 原语(屏蔽 children/cleanup-frame 收集);RxList.length、RxMap keys/values/entries/size、RxSet.size 全部惰性创建,destroy 只清理真正创建过的。旧 FIXME("不能惰性化")成因即 children 误收集,已回归覆盖。
- **B3**:map 行级依赖探测——每个 map 产物复用一个探测 effect(`MapItemDependencyProbe`),mapFn 仍恰好执行一次;无依赖行零对象分配(共享 frozen 空 frame),有依赖/子 effect 的行经 `transferCapturesTo` 升级为常驻行级 Computed(订阅原样转移,不重跑 mapFn)。
- **B4/B7**:getter context 按实例复用(filter 每行每次重算省 1 对象 + 1 闭包 + 2 bind);RxMap iterator 与 forEach 对齐(原生迭代器);RxSet 集合运算直接迭代;toSorted patch 二分定位删除(indexOf 兜底);`util.replace` 的 O(n²) key diff 改 Set。

### 8.3 行为与协议变更说明(下游可见面)

1. `TriggerInfo` 形状、`argv` 结构、公开 API 签名均未变;`Notifier.trigger` 签名不变。
2. **内部协议**变化:`triggerEffects`/`triggerEffect`/`scheduleEffect` 参数由 `(dep/effect, info)` 变为 `(dep/effect, source, type, inputInfo)`;`triggerPrimitiveAtomValue` 改为标量参数 `(source, newValue, oldValue)`。axii/axle 均未引用这些内部方法(已 grep 验证)。
3. 非 patch 型 computed(无 `applyPatch`、调度器未声明第三参)不再收到 `triggerInfos`——此前它们收到后也只是被 fullRecompute 清空,无可观察差异;`onTrack`/`onTrigger` 等调试钩子在有监听者时行为不变。
4. `forEach`/`for...of` 的 `onTrack` 观察粒度从"每 index + ITERATE"变为"仅 ITERATE";响应行为(何时重跑)不变。
5. RxList/RxMap/RxSet 的派生 meta(length/keys/values/entries/size)从"构造即创建"变为"首次访问创建";`RxMap.keys/values/entries` 从实例函数字段变为原型方法(调用形式不变)。
6. 修正的存量 bug:RxList `for...of` 死循环;async computed 出错且无人持有 cleanPromise 时的 unhandled rejection;10 万行 `replaceData` 的 RangeError。

### 8.4 后续(Track C,未实施)

维持方案 §4-Track C 的结论:TriggerInfo 的 dep 级消费者标志(C1)、版本计数依赖追踪(C2)、lazy pull(C3)都是内核数据结构级手术,收益上限高但必须在独立实验分支上以全量 bench + axii/axle 测试矩阵验证。本轮 chain-10(8.9µs vs 7µs 目标)与 at() tracked(17.4µs vs 10µs 目标)两项未达标的剩余成本正是 C 类改动的目标区间,建议作为下一轮工作启动。

> 勘误:B2 提交(`perf(RxList): prune dead index deps...`)commit message 中的
> "640ms -> 6ms" 为撰写时的估算笔误,准确实测为本表所列 shift+unshift 场景 801ms → 13ms。
